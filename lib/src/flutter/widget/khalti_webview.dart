...
import 'package:khalti_sdk/src/flutter/data/core/exception_handler.dart';
import 'package:khalti_sdk/src/flutter/strings.dart';
import 'package:khalti_sdk/src/flutter/util/utils.dart';
import 'package:khalti_sdk/src/flutter/widget/khalti_pop_scope.dart';
        ...
        children: [
          ValueListenableBuilder(
            ...
            builder: (_, showLoader, __) {
              return showLoader ? const LinearProgressIndicator(color: Color(0xFFDC0019)) : const SizedBox.shrink();
            },
          ),
          Expanded(
            child: SafeArea(
                  ...
                  switch (connectionStatus) {
                    case InternetStatus.connected:
                      return FutureBuilder<PaymentDetailModel>(
                        ...
                        builder: (context, snapshot) {
                          if (snapshot.hasData) {
                            debugPrint('✅✅✅✅✅✅✅✅✅✅ ************ $webViewControllerCompleter ');
                            debugPrint('✅✅✅✅✅✅✅✅✅✅ ************ ${ snapshot.data?.returnUrl} ');
...
class _KhaltiWebViewClient extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return KhaltiPopScope(
      ...
      child: InAppWebView(
        // 1. Add this handler to intercept the navigation before it happens
        shouldOverrideUrlLoading: (controller, navigationAction) async {
          final webUri = navigationAction.request.url;
          if (webUri != null && returnUrl.isNotNullAndNotEmpty) {
            final currentStringUrl = webUri.toString();

            // Check if the URL is the return_url
            if (currentStringUrl.contains(returnUrl!)) {
              ...
              // 2. RETURN CANCEL: This stops the WebView from loading the return_url
